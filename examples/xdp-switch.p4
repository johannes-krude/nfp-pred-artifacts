...
parser ParserImpl(packet_in packet, out headers hdr){// , inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".parse_ethernet") state parse_ethernet {
        ...
        transition select(hdr.ethernet.etherType[15:8]) {
            //16w0 &&& 16w0xfe00: parse_llc_header;
            //16w0 &&& 16w0xfa00: parse_llc_header;
            8w0x00: parse_llc_header;
            8w0x01: parse_llc_header;
            8w0x04: parse_llc_header;
            8w0x05: parse_llc_header;
            default: parse_ethernet2;
        }
    }
    @name(".parse_ethernet2") state parse_ethernet2 {
    ...
    @name(".parse_fabric_payload_header") state parse_fabric_payload_header {
        ...
        transition select(hdr.fabric_payload_header.etherType[15:8]) {
            //16w0 &&& 16w0xfe00: parse_llc_header;
            //16w0 &&& 16w0xfa00: parse_llc_header;
            8w0x00: parse_llc_header;
            8w0x01: parse_llc_header;
            8w0x04: parse_llc_header;
            8w0x05: parse_llc_header;
            default: parse_fabric_payload_header2;
        }
    }
    @name(".parse_fabric_payload_header2") state parse_fabric_payload_header2 {
    ...
    @name(".parse_gre") state parse_gre {
        ...
        transition select(hdr.gre.word, hdr.gre.proto) {
            (16w0x2000, 16w0x6558): parse_nvgre;
            (16w0x0000, 16w0x800): parse_gre_ipv4;
            (16w0x0000, 16w0x86dd): parse_gre_ipv6;
            (16w0x0000, 16w0x22eb): parse_erspan_t3;
            default: accept;
        }
    }
    ...
    @name(".parse_icmp") state parse_icmp {
        ...
        transition select(hdr.icmp.typeCode[15:8]) {
            //16w0x8200 &&& 16w0xfe00: parse_set_prio_med;
            8w0x82: parse_set_prio_med;
            8w0x83: parse_set_prio_med;
            //16w0x8400 &&& 16w0xfc00: parse_set_prio_med;
            8w0x84: parse_set_prio_med;
            8w0x85: parse_set_prio_med;
            8w0x86: parse_set_prio_med;
            8w0x87: parse_set_prio_med;
    ...
    @name(".parse_inner_ipv4") state parse_inner_ipv4 {
        ...
        transition select(3w0 ++ hdr.inner_ipv4.fragOffset, 4w0 ++ hdr.inner_ipv4.ihl, hdr.inner_ipv4.protocol) {
            (3w0 ++ 13w0x0, 4w0 ++ 4w0x5, 8w0x1): parse_inner_icmp;
            (3w0 ++ 13w0x0, 4w0 ++ 4w0x5, 8w0x6): parse_inner_tcp;
            (3w0 ++ 13w0x0, 4w0 ++ 4w0x5, 8w0x11): parse_inner_udp;
            default: accept;
        }
    }
    ...
    @name(".parse_int_header") state parse_int_header {
        ...
        transition select(3w0 ++ hdr.int_header.rsvd1, hdr.int_header.total_hop_cnt) {
            (3w0 ++ 5w0x0, 8w0x0): accept;
            default: parse_int_header2;
        }
    }
    @name(".parse_int_header2") state parse_int_header2 {
...
//            //(5w0x0 &&& 5w0xf, 8w0x0 &&& 8w0x0): parse_int_val;
    ...
    @name(".parse_int_val") state parse_int_val {
        packet.extract(hdr.int_val[0]);//.next);
        transition select(hdr.int_val[0].bos) {//.last.bos) {
            1w0: parse_int_val1;
    ...
    @name(".parse_int_val") state parse_int_val1 {
        packet.extract(hdr.int_val[1]);//.next);
        transition select(hdr.int_val[1].bos) {//.last.bos) {
//            1w0: parse_int_val2;
            1w1: parse_inner_ethernet;
            default: accept;
        }
    ...
    @name(".parse_ipv4") state parse_ipv4 {
        ...
        transition select(3w0 ++ hdr.ipv4.fragOffset, 4w0 ++ hdr.ipv4.ihl, hdr.ipv4.protocol) {
            (3w0 ++ 13w0x0, 4w0 ++ 4w0x5, 8w0x1): parse_icmp;
            (3w0 ++ 13w0x0, 4w0 ++ 4w0x5, 8w0x6): parse_tcp;
            (3w0 ++ 13w0x0, 4w0 ++ 4w0x5, 8w0x11): parse_udp;
            (3w0 ++ 13w0x0, 4w0 ++ 4w0x5, 8w0x2f): parse_gre;
            (3w0 ++ 13w0x0, 4w0 ++ 4w0x5, 8w0x4): parse_ipv4_in_ip;
            (3w0 ++ 13w0x0, 4w0 ++ 4w0x5, 8w0x29): parse_ipv6_in_ip;
            (3w0 ++ 13w0, 4w0 ++ 4w0, 8w2): parse_set_prio_med;
            (3w0 ++ 13w0, 4w0 ++ 4w0, 8w88): parse_set_prio_med;
            (3w0 ++ 13w0, 4w0 ++ 4w0, 8w89): parse_set_prio_med;
            (3w0 ++ 13w0, 4w0 ++ 4w0, 8w103): parse_set_prio_med;
            (3w0 ++ 13w0, 4w0 ++ 4w0, 8w112): parse_set_prio_med;
            default: accept;
        }
    }
    ...
    @name(".parse_lisp") state parse_lisp {
        ...
        byte_t byte = packet.lookahead<byte_t>();
        transition select(byte.value[7:4]) {//(packet.lookahead<bit<4>>())[3:0]) {
    ...
    @name(".parse_mpls") state parse_mpls {
        packet.extract(hdr.mpls[0]);//.next);
        transition select(hdr.mpls[0].bos) {//.last.bos) {
            1w0: parse_mpls1;
            1w1: parse_mpls_bos;
            default: accept;
        }
    ...
    @name(".parse_mpls") state parse_mpls1 {
        packet.extract(hdr.mpls[1]);//.next);
        transition select(hdr.mpls[1].bos) {//.last.bos) {
            1w0: parse_mpls2;
            1w1: parse_mpls_bos;
            default: accept;
        }
    ...
    @name(".parse_mpls") state parse_mpls2 {
        packet.extract(hdr.mpls[2]);//.next);
        transition select(hdr.mpls[2].bos) {//.last.bos) {
            //1w0: parse_mpls2;
            1w1: parse_mpls_bos;
            default: accept;
        }
    ...
    @name(".parse_mpls_bos") state parse_mpls_bos {
        byte_t byte = packet.lookahead<byte_t>();
        transition select(byte.value & 0xf0) {//(packet.lookahead<bit<4>>())[3:0]) {
            8w0x40: parse_mpls_inner_ipv4;
            8w0x60: parse_mpls_inner_ipv6;
    ...
    @name(".parse_vxlan_gpe") state parse_vxlan_gpe {
        ...
        transition select(hdr.vxlan_gpe.flags>>7, hdr.vxlan_gpe.next_proto) {
            //(8w0x8 &&& 8w0x8, 8w0x5 &&& 8w0xff): parse_gpe_int_header;
            (8w0x1, 8w0x5): parse_gpe_int_header;
...
ebpfFilter(ParserImpl(), pipe()) main;
