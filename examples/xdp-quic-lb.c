...
int act_main(struct xdp_md *ctx) {
	...
	if (data + sizeof(*eth) + sizeof(*ip) + sizeof(*udp) + 1 > data_end)
		return XDP_DROP;
	first_byte = ((char *) (data + sizeof(*eth) + sizeof(*ip) + sizeof(*udp)))[0];
	...
	if (first_byte & 0x80) {
		dcid_off = 6;
		if (data + sizeof(*eth) + sizeof(*ip) + sizeof(*udp) + 6 > data_end)
			return XDP_DROP;
		dcid_len = ((char *) (data + sizeof(*eth) + sizeof(*ip) + sizeof(*udp) + 5))[0];
		if (!dcid_len)
			return XDP_PASS;
		if (data + sizeof(*eth) + sizeof(*ip) + sizeof(*udp) + 6 + dcid_len > data_end)
			return XDP_DROP;
	}

	if (data + sizeof(*eth) + sizeof(*ip) + sizeof(*udp) + dcid_off + 1 > data_end)
		return XDP_DROP;
	dcid = data + sizeof(*eth) + sizeof(*ip) + sizeof(*udp) + dcid_off;
	first_octet = dcid[0];
	...
	if (len < 8)
		return XDP_PASS;
	if (data + sizeof(*eth) + sizeof(*ip) + sizeof(*udp) + dcid_off + 9 > data_end)
		return XDP_DROP;
	routing = 0;
